        ...
        <table class="table">
            <thead>
                ...
                <?php for ($i = 1; $i <= 7; $i++) { 
                    $test_num = 21+$i;
                    $num_list = 1;
                    $res_user = mysqli_query($user_db, "select * from test.user_result where test_id = '${test_num}' ");
                        $name_row = mysqli_fetch_assoc($res_user);
                        if ($name_row['id'] != ''){
                    ?>
                <tr>
                    <th colspan="4" class="text-center">Тест <?php echo $i ?></th>
            ...
            while ($row = mysqli_fetch_assoc($res)) {
                ...
                <tr>
                    ...
                    <td><?php echo date('Y-m-d',strtotime($row['datetime_test']));?> <?php echo date('h:i:s',strtotime($row['datetime_test']));?></td>
                </tr>
            <?php } ?>
            </tbody>
            <?php } ?>
